<?php
namespace BK_checkout_np;
class BK_checkout 
{
	// URL checkout của baokim.vn
	private $baokim_url = 'https://www.baokim.vn/payment/customize_payment/order';
	
	private $api_username = 'bishidocom';

	private $api_password = 'FQbuvm8PGM5MerKEHeWW';
	// Mã merchante site 
	private $merchant_id = '19809';	// Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site

	// Mật khẩu bảo mật
	private $secure_pass = 'eabbf9b9d54275df'; // Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site
	 ...
	 */
	public function verifyResponseUrl($input = array())
	{
		$checksum = $input['checksum'];
		unset($input['checksum']);
		ksort($input);
		$str_combined = $this->secure_pass.implode('', $input);
